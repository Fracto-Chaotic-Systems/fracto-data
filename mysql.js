...
const format_sql = (sql) => {
  ...
  return formatted.replace(
    /\s+(FROM|WHERE|ORDER BY|LIMIT|OFFSET|VALUES|SET|AND|OR)\s+/g,
    "\n  $1 ",
  );
};
...
const sql_segments = (statement) => {
  ...
  const keyword_pattern =
    /\b(SELECT|FROM|WHERE|ORDER BY|LIMIT|OFFSET|INSERT INTO|VALUES|UPDATE|SET|DELETE FROM|AND|OR)\b|(?<!\w)-?\d+(?:\.\d+)?(?!\w)/g;
  let cursor = 0;
  let match;
  while ((match = keyword_pattern.exec(statement))) {
    if (match.index > cursor)
      segments.push({
        text: statement.slice(cursor, match.index),
        color: null,
      });
    segments.push({
      text: match[0],
  ...
  if (cursor < statement.length)
    segments.push({ text: statement.slice(cursor), color: null });
  return segments;
};
...
const log_sql = (sql) => {
  ...
  console.log(
    `${STRUCTURED_LOG_PREFIX}${JSON.stringify({
      kind: "sql",
      statement,
...
export const select = (connection, query, cb) => {
  ...
  connection.query(sqlQuery, (error, results) => {
    if (error) {
      ...
      console.log(
        "select returned",
        chalk.hex("#d4a72c")(results.length),
        "records",
      );
      cb(results);
    }
