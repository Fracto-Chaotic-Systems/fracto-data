...
const approximate_radius = (radius, start_z, coefflist, path_factor) => {
  ...
  for (
    let re_offset = -DIVISIONS / 2;
    re_offset < DIVISIONS / 2;
    re_offset += 1
  ) {
    for (
      let im_offset = -DIVISIONS / 2;
      im_offset < DIVISIONS / 2;
      im_offset += 1
    ) {
      const offset = new Complex(increment * re_offset, increment * im_offset);
