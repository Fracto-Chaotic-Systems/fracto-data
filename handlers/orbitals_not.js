...
const orbital_recurse = (
  ...
  if (seed_1.magnitude() < MAX_MAGNITUDE) {
    ...
    if (all_points[seed_1_str]) {
      console.log(
        `found cycle: ${iteration - all_points[seed_1_str].iteration}`,
      );
      return true;
    }
  ...
  if (seed_2.magnitude() < MAX_MAGNITUDE) {
    ...
    if (all_points[seed_2_str]) {
      console.log(
        `found cycle: ${iteration - all_points[seed_2_str].iteration}`,
      );
      return true;
    }
...
const inverse_recurse = (
  ...
  if (iteration > max_depth) {
    const point_list = point_path.map(
      (point) => `${point.scalar > 0 ? "plus" : "minus"}: ${point.point_str}`,
    );
  ...
  const found_point = point_path.find(
    (p) => p.point.re === new_seed.re && p.point.im === new_seed.im,
  );
  if (found_point) {
    const point_list = point_path.map(
      (point) => `${point.scalar > 0 ? "plus" : "minus"}: ${point.point_str}`,
